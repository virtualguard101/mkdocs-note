...
class NoteProcessor:
	...
	def _parse_timezone(self, timezone_str: str) -> timezone:
		...
		try:
			...
			self.logger.warning(
				f"Error parsing timezone {timezone_str}: {e}, using UTC+0"
			)
			return timezone.utc
		...
		try:
			...
			if self.config.use_git_timestamps:
				...
				if git_time:
					...
					self.logger.debug(
						f"Using Git commit time for {file_path.name}: {modified_date}"
					)
				else:
					...
					self.logger.debug(
						f"Git time unavailable, using file system time for {file_path.name}: {modified_date}"
					)
			else:
				...
				modified_date = self._format_timestamp(stat.st_mtime)
				self.logger.debug(
					f"Using file system time for {file_path.name}: {modified_date}"
				)
	...
	def _extract_title_from_notebook(self, file_path: Path) -> Optional[str]:
		...
		try:
			...
			self.logger.warning(
				f"Failed to extract title from notebook {file_path}: {e}"
			)
	...
	def _extract_title_from_markdown(self, file_path: Path) -> Optional[str]:
		...
		try:
			...
			self.logger.warning(
				f"Failed to extract title from markdown {file_path}: {e}"
			)
	...
	def _get_git_commit_time(self, file_path: Path) -> Optional[float]:
		...
		try:
			...
			if result.returncode == 0 and result.stdout.strip():
				...
				self.logger.debug(
					f"Git log failed for {relative_path}: {result.stderr}"
				)
				return None
...
class CacheManager:
	...
	def _calculate_notes_hash(self, notes: List[NoteInfo]) -> str:
		...
		for note in notes:
			notes_info.append(
				f"{note.file_path.name}:{note.modified_time}:{note.file_size}"
			)
...
class RecentNotesUpdater:
	...
	def update(self) -> bool:
		...
		try:
			...
			if success:
				self.logger.debug(
					f"Successfully updated recent notes ({len(recent_notes)} notes)"
				)
			else:
