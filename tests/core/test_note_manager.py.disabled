...
from mkdocs_note.utils.docsps.handlers import (
	NoteProcessor,
	CacheManager,
	IndexUpdater,
	RecentNotesUpdater,
...
class TestNoteProcessor(unittest.TestCase):
	...
	def test_process_note_success(self, mock_stat):
		...
		with patch.object(self.processor, "_extract_title", return_value="Test Note"):
			with patch.object(
				self.processor, "_generate_relative_url", return_value="test/note/"
			):
	...
	def test_extract_title_markdown(self):
		...
		with patch.object(
			Path, "open", mock_open(read_data="# My Test Note\n\nContent here")
		):
	...
	def test_extract_title_markdown_no_title(self):
		...
		with patch.object(
			Path, "open", mock_open(read_data="No title here\n\nContent here")
		):
			result = self.processor._extract_title_from_markdown(file_path)
	...
	def test_extract_title_notebook_no_title(self):
		...
		notebook_data = {
			"cells": [
				{"cell_type": "markdown", "source": ["No title here\n", "Content here"]}
			]
		}
...
class TestRecentNotesUpdater(unittest.TestCase):
	...
	@patch("mkdocs_note.utils.docsps.handlers.NoteScanner.scan_notes")
	...
	@patch("mkdocs_note.utils.docsps.handlers.IndexUpdater.update_index")
	@patch("mkdocs_note.utils.docsps.handlers.CacheManager.should_update_notes")
	@patch("mkdocs_note.utils.docsps.handlers.NoteProcessor.process_note")
	@patch("mkdocs_note.utils.docsps.handlers.NoteScanner.scan_notes")
